...
async function generateRewrite(input) {
	...
	try {
		...
		if (terminalError !== void 0) return {
			ok: false,
			code: timedOut ? "timeout" : "llm-failed",
...
var RewriteService = class extends Service {
	...
	async rewrite(text, signal) {
		...
		if (!isModelSelectionConfigured(selection)) return {
			ok: false,
			code: "no-model",
			message: "尚未配置改写模型，请在设置中选择"
		};
		const scenes = detectScenes(text);
		const system = buildSystemPrompt(scenes.map((id) => this.playbookContentById.get(id)).filter((content) => content !== void 0));
		...
		return {
			ok: true,
			rewritten: recombine(result.text, attachments),
			scenes
		};
	}
};
